...
const GRAY = "#737373";
const RULE = "#d4d4d4";
const ACCENT = "#0040ff"; // used sparingly
        ...
        <div className="pt-36 md:pt-40 px-6 md:px-10">
          <div className="grid grid-cols-12 gap-6 items-end">
            <div className="col-span-12 md:col-span-7 lg:col-span-8">
              <Mono className="text-[#737373]">{scenes[0].eyebrow}</Mono>
              <motion.h1
                style={{ y: yTitle }}
                className="font-magazine font-light text-[#0a0a0a] mt-6 leading-[0.92] tracking-[-0.02em]"
              >
                <span className="block text-[clamp(48px,9vw,148px)]">{brand.nameHe}.</span>
                <span className="block text-[clamp(28px,4.6vw,76px)] text-[#737373] font-light mt-3">
                  {scenes[0].title}
                </span>
              </motion.h1>
            </div>
        ...
        <div className="px-6 md:px-10 mt-12 md:mt-20">
          ...
          <div className="grid grid-cols-12 gap-6 py-8" dir="ltr">
            ...
            <div className="col-span-12 md:col-span-4 flex flex-col justify-end" dir="rtl">
              <p className="font-sans text-[14px] leading-[1.7] text-[#0a0a0a] max-w-[28ch]">
                {scenes[0].body}
              </p>
            </div>
          </div>
      ...
      <div className="px-6 md:px-10 py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-0">
          {stats.map((s, i) => (
            <div
              key={i}
              className={`p-6 md:p-10 border-[#d4d4d4] ${i !== 0 ? "border-l" : ""} ${i < 2 ? "border-b md:border-b-0" : ""}`}
            >
              <Mono className="text-[#737373]">{String(i + 1).padStart(2, "0")} / 04</Mono>
              <div className="mt-4 font-magazine font-light text-[clamp(48px,7vw,96px)] leading-none text-[#0a0a0a]">
                {s.value}
                <span className="text-[#737373]">{s.suffix}</span>
              </div>
              <div className="mt-4 font-sans text-[14px] text-[#0a0a0a]">{s.label}</div>
              <div className="mt-1 font-mono text-[10px] tracking-[0.18em] uppercase text-[#737373]">{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
